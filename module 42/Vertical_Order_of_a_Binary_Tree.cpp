...
void varticalOrderTraversal(BinaryTree* root, int distnc,  map<int, vector<int>> &M){
    if(root == NULL) return;
    M[distnc].push_back(root->data);
    varticalOrderTraversal(root->LeftChild, distnc-1, M);
    varticalOrderTraversal(root->RighChild, distnc+1, M);
}
...
cout<<endl<<endl;
    ...
    varticalOrderTraversal(root, 0, M);
